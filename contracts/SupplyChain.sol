...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier sold(uint sku) {
    Item memory item = items[sku];
    require(item.state == State.Sold && item.seller != address(0));
    _;
  }

  modifier shipped(uint sku) {
    Item memory item = items[sku];
    require(item.state == State.Shipped && item.seller != address(0));
    _;
  }

  modifier received(uint sku) {
    Item memory item = items[sku];
    require(item.state == State.Received && item.seller != address(0));
    _;
  }
